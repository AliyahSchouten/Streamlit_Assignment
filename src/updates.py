...
# Create a .csv file:
Base().df.to_csv(f'{folder_dir}\\oracle_students.csv', index=False)

# Read in the data to a dataframe object:
df = pd.read_csv(f'{folder_dir}\\oracle_students.csv', low_memory=False)
...
df.to_csv(f'{folder_dir}\\oracle_students.csv', index=False)
